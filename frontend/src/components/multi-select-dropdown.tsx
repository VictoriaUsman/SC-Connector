...
export interface DropdownGroup<T extends DropdownOption = DropdownOption> {
  label: string;
  options: T[];
}

function OptionRow<T extends DropdownOption>({
  item,
  checked,
  onToggle,
  renderOption,
  ...
  return (
    <label className="flex items-start gap-2.5 cursor-pointer rounded-md px-2 py-1.5 hover:bg-accent text-sm">
      <Checkbox
        checked={checked}
      ...
      <div className="flex-1 min-w-0">
        <div>{renderOption ? renderOption(item) : item.label}</div>
        {item.description && (
...
export function MultiSelectDropdown<T extends DropdownOption>({
  ...
  const selectedItems = useMemo(
    () => allOptions.filter((o) => selected.includes(o.id)),
    [allOptions, selected],
  );
          ...
          <div className="max-h-72 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              ...
              filteredGroups.map((group) => (
                <div key={group.label}>
                  <div className="px-2 pt-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                    {group.label}
                  </div>
