...
**Exactly-once scheduling (no duplicate pulls)**: Cloud Scheduler delivers at-least-once and a slow run can overlap the next cron tick, so a due schedule could be observed by two scheduler invocations and fanned out twice — producing duplicate Drive files. Two guards prevent this: (1) the scheduler **atomically claims** each due schedule before fanning out via `firestore_utils.claim_due_schedule()`, which advances `next_run_at` inside a transaction that only proceeds if the schedule is still due (the loser skips it); (2) each workflow launch in `launch_for_marketplace()` carries a deterministic idempotency key (`trigger|schedule|execution_date|client|marketplace|report_type|api_source|date-range|variant`) recorded via `firestore_utils.try_claim_job_launch()` (atomic `document.create()` in `_job_launch_dedupe`), so a repeated launch of the same logical pull is skipped instead of producing a duplicate file. The `trigger` scope means a deliberate manual "Run Now" is never deduped against the scheduled run, only against an identical duplicate. `scripts/scan_duplicate_drive_files.py` is a read-only tool that flags any already-duplicated files in Drive for Ops cleanup.
...
**Workflow Error Handling**: The workflow YAML uses a global try/except pattern — `main` calls a `report_pipeline` subworkflow, and any unhandled error (auth failure, create_report error, etc.) is caught by the global handler which marks the job as `"failed"` using `args.job_id`, via a PostgREST PATCH against Supabase's `jobs` table. This prevents zombie "pending" jobs. Error serialization uses `json.encode(e)` (not `string(e)`, which crashes on dicts). The workflow service account has `roles/secretmanager.secretAccessor` scoped to just the Supabase service-role key secret (not project-wide) to authenticate that PATCH.
...
- Launch idempotency guard: every fan-out path (`removed`, `api_call`, and the normal per-pull loop) first calls `try_claim_job_launch()` with a deterministic dedupe key, so a duplicate scheduler fire or double-clicked manual trigger cannot launch the same pull twice (defends against duplicate Drive files). Fails open on Firestore errors
...
- **Per-SKU breakdown** is gated per bot config by `_sku_breakdown_enabled_for_config(config)`: the per-customer `sku_breakdown_enabled` toggle (set in the Slack Bots UI, persisted on the `bot_configs` doc) OR-ed with the legacy family allowlist (`_sku_breakdown_enabled`, Skylight/Ritual, env-overridable via `SKU_BREAKDOWN_CLIENT_IDS`). The toggle lets a PM enable the breakdown for any customer; the allowlist is a transitional safety-net default (so allowlisted accounts keep working before they're toggled, and can't be silently disabled by a stale `false`) and can be retired once those configs are toggled on. A combined family appends one per-marketplace breakdown for each marketplace in `_sku_breakdown_marketplaces()` (default US, CA, UK in display order; override via `SKU_BREAKDOWN_MARKETPLACES`). Each section is built from `_query_sku_breakdown(member_client_id, [marketplace], now)` and reconciled to that marketplace's own single-currency account line (USD/CAD/GBP), with the marketplace shown in the section header. Members without it enabled, marketplaces the family doesn't cover, or any reconciliation miss are silently skipped — every section is additive and independent. The orders/SKU day window is computed by the shared `_marketplace_day_window()` helper (live day-to-date for hourly, full calendar day for recaps), so the account and SKU queries provably share one window
- **Day-end recap breakdown**: the midnight "Day N Recap" also appends a per-SKU breakdown (same `_sku_breakdown_enabled_for_config` gate) by reusing `_append_sku_breakdown` with `full_day=True` + `report_date=recap_date`. The recap stays per-config (one recap per account, single-currency), so each regional account's breakdown lands in its own recap message (all threaded under the same day anchor); it covers the completed recap day only (no cumulative SKU rows). The year-round `daily_recap` bot is untouched
